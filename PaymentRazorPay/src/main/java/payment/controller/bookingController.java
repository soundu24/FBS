...
import payment.model.Booking;
import payment.model.Passenger;
import payment.repository.BookingRepository;
...
public Booking updateBooking(@RequestBody Booking booking,@PathVariable("booking_id") long pnr,@PathVariable("mailId") String mailId) {
	...
	book_repo.save(dbResponse);
	 ...
	 if(booking.checked_in==true) {System.out.println("checked in");log.info("Updated CheckIn Status");
	 //return emailService.checkIn(dbResponse,mailId);
	 }
	  if(booking.booking_cancelled==true)
	  {booking.setChecked_in(false);System.out.println("Booking Cancelled");log.info("Updated Booking Cancelled Status");
